...
final public class ManifezzClass: CrawlMeister
{
    ...
    private final class ManifezzController:NSObject,CustomControllable {
        ...
        let tunesTable = ["china","elizabeth","whipping","one More","riders","light"]
        ...
        fileprivate func createMarkDown(_ aurl: String, _ cont: ManifezzClass.ManifezzContext) {
            ...
            if  let u = URL(string:aurl){
                ...
                if let songName = cont.name?.lowercased() {
                    for tuneTag in tunesTable {
                        if songName.hasPrefix(tuneTag) {
                            tags.append(tuneTag)
                            break
                        }
                    }
